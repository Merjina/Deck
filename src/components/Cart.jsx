...
const Cart = ({ cartItems }) => {
  ...
  const updateCartQuantity = async (itemId, action) => {
    try {
      ...
      await axios.put(`http://localhost:8081/api/cart/update/${itemId}`, { action });
  ...
  const removeFromCart = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:8081/api/cart/remove/${itemId}`);
            ...
            {cartItems.map((item) => (
              <Col md={4} key={item.id} className="mb-4">
                <Card className="bg-dark text-light product-card">
                  <Card.Img
                    variant="top"
                    src={`http://localhost:8081/api/products/images/${item.imagePath}`}
